...
def create_employee(randEmployeeCreation)
  ...
  1..randEmployeeCreation.each do |i|
    tmp_user =
      User.create(
        ...
        created_at:
          Time.at(
            (Time.local(2017, 7, 8).to_f - Time.local(2019, 7, 8).to_f) * rand +
              Time.local(2020, 7, 8).to_f
          ),
...
def create_customer(randCustomerCreation)
  1..randCustomerCreation.each do |i|
    ...
    tmp_user =
      User.create(
        ...
        created_at:
          Time.at(
            (Time.local(2017, 7, 8).to_f - Time.local(2020, 7, 8).to_f) * rand +
              Time.local(2020, 7, 8).to_f
          ),
        password: Faker::Internet.unique.password
      )

    tmp_customer_address =
      Address.create(
        type_of_address: %w[House Apartment Commercial Corporate Hybrid].sample,
    ...
    tmp_lead =
      Lead.create(
        ...
        created_at:
          Time.at(
            (tmp_user.created_at.to_f - Time.local(2020, 7, 8).to_f) * rand +
              Time.local(2020, 7, 8).to_f
          )
      )

    1..rand(1..4).each do |j|
      tmp_building_address =
        Address.create(
          type_of_address: %w[
            House
            Apartment
      ...
      tmp_battery =
        Battery.create(
          building_id: tmp_building.id,
          battery_type: %w[Residential Commercial Corporate Hybrid].sample,
          battery_status: 'ACTIVE',
          date_of_commissioning:
            Time.at(
              (tmp_user.created_at.to_f - Time.local(2018, 7, 8).to_f) * rand +
                Time.local(2018, 7, 8).to_f
            ),
          date_of_last_inspection:
            Time.at(
              (tmp_building.created_at.to_f - Time.local(2020, 7, 8).to_f) *
                rand + Time.local(2020, 7, 8).to_f
            ),
    ...
    1..rand(1..4).each do |j|
      tmp_quote =
        Quote.create(
          ...
          unit_price: rand(7565..100_000),
          install_fee: rand(2000..10_000),
          total_price: rand(2000..1_000_000),
          elevator_number: rand(1..50).floor,
          product_line: %w[Excelium Premium Standard].sample,
      ...
      elsif tmp_battery.battery_type == 'Corporate' ||
            tmp_battery.battery_type == 'Hybrid'
        tmp_quote.update('businesses' => rand(1..20).floor)
        tmp_quote.update('parking_spaces' => rand(1..140).floor)
...
Building.all.each do |b|
  ...
  b.update_attributes(
    technical_contact_full_name: employee2.first_name + employee2.last_name
  )
  b.update_attributes(technical_contact_email: employee2.email)
  b.update_attributes(technical_contact_phone: employee2.phone)

  b.update_attributes(
    administrator_full_name: employee1.first_name + employee1.last_name
  )
